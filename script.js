...
const coffeeStatus = _ => {
  ...
  else if (data.producers[10].qty > 0) status.innerHTML = 'Swim in the sweet,<br>sweet nectar of the Gods!';
  ...
  else if (data.producers[7].qty > 0) status.innerHTML = 'One step closer to<br>COFFEE DOMINATION!!!';
  else if (data.producers[6].qty > 0) status.innerHTML = 'One small shop for man...<br>One giant leap for yo heart rate!';
  else if (data.producers[5].qty > 0) status.innerHTML = 'For when one gallon<br>just won\'t do!';
  ...
  else if (data.producers[2].qty > 0) status.innerHTML = 'The journey of a billion<br>coffees begins with<br>a single sip.';
...
function makeProducerDiv(producer) {
  ...
  const html = `
  <div id="left-side-prod" class="producer-column">
    <div class="producer-title">${displayName}</div>
    <div id="prodbutton"><button type="button" id="buy_${producer.id}">Buy</button></div>
  </div>
